...
def verificar_viaje_pendiente(id_chofer: int, db: Session = Depends(get_db)):
    chofer_db = db.query(models.Choferes).filter(models.Choferes.id_chofer == id_chofer).first()
    estado_actual = chofer_db.estado_chofer.name if chofer_db else "DESCONECTADO"
    ...
    if viaje_db:
        return JSONResponse({"viaje_pendiente": True, "id_viaje": viaje_db.id_viaje, "estado_chofer": estado_actual})
    return JSONResponse({"viaje_pendiente": False, "estado_chofer": estado_actual})