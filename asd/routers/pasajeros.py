...
def recarga(
    ...
    if not ref_limpia.isdigit() or len(ref_limpia) != 10:
        return templates.TemplateResponse(
            ...
            context={
                "id_pasajero": id_pasajero,
                "error": "El número de referencia debe contener exactamente 10 dígitos numéricos."
            }
        )
    
    referencia_existente = db.query(models.HistorialRecargas).filter(models.HistorialRecargas.numero_referencia == ref_limpia).first()
    ...
    if referencia_existente:
        return templates.TemplateResponse(
            ...
            context={
                "id_pasajero": id_pasajero,
                "error": "Esta referencia ya ha sido procesada anteriormente. Verifique sus datos."
            }
        )
