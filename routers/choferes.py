...
def registrar_contacto(
    ...
    num_limpio = numero.strip()
    
    if not num_limpio.isdigit() or len(num_limpio) != 7:
        cantidad_contactos = db.query(models.AgendaContactos).filter(models.AgendaContactos.id_chofer == id_chofer).count()
        return templates.TemplateResponse(
            request=request,
            name="contactos.html",
            context={
                "id_chofer": id_chofer,
                "contactos": cantidad_contactos,
                "error": "El número de teléfono debe contener exactamente 7 dígitos numéricos."
            }
        )
    
    numero_telefonico = f"{numeral}-{num_limpio}"
...
def registrar_vehiculo(
    ...
    if vehiculo_existente:
        return templates.TemplateResponse(
            ...
            context={
                "id_chofer": id_chofer,
                "error": f"La placa {matricula_formateada} ya se encuentra registrada en el sistema."
            }
        )
